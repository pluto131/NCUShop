...
if(a.equals("insert")){
    // 执行插入商品入库模块,表名：shangpinruku
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                                                    
    String charuid = new CommDAO().insert(request,"shangpinruku",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update shangpinxinxi set shangpinkucun=shangpinkucun+'"+request.getParameter("rukushuliang")+"' where id='"+request.getParameter("shangpinxinxiid")+"'");
    ...
    // 执行更新商品入库模块,表名：shangpinruku
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                                                                                                                                                        

    String charuid = new CommDAO().update(request,"shangpinruku",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("shangpinruku").find(Request.get("id"));
    
    new CommDAO().delete(request,"shangpinruku");  // 执行删除命令
